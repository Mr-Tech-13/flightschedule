...
app.patch('/api/flights/:id', allow('admin','scheduler'), async(req,res,next)=>{try{const f=req.body;const r=await pool.query(`UPDATE flights SET airline=COALESCE($2,airline),arrival_number=$3,departure_number=$4,origin=$5,destination=$6,arrival_at=$7,departure_at=$8,gate=$9,notes=$10,priority=$11,ferry=$12 WHERE id=$1 RETURNING *`,[req.params.id,f.airline?.toUpperCase()||null,f.arrivalNumber||null,f.departureNumber||null,f.origin?.toUpperCase()||null,f.destination?.toUpperCase()||null,f.arrivalAt||null,f.departureAt||null,f.gate||null,f.notes||null,!!f.priority,!!f.ferry]);if(!r.rows.length)return res.status(404).json({error:'Flight not found'});res.json(r.rows[0])}catch(e){next(e)}});
