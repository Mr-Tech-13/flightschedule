    ...
    for (const flight of flights) {
      ...
      for (const role of ['lead','agent']) {
        ...
        for (let n = current.filter(a => a.role === role).length; n < required; n++) {
          const choices = employees.filter(e => eligible(e, role, flight.airline) && new Date(e.starts_at) <= win.start && new Date(e.ends_at) >= win.end && !(busy.get(e.id) || []).some(b => overlaps(b, win)))
...
function eligible(e, role, airline) {
